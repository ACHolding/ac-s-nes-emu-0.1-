...
struct Cartridge {
    ...
    bool loaded = false;
    uint16_t mapper = 0;
    int mirrorMode = 0; // 0: horizontal, 1: vertical, 2: single-screen low, 3: single-screen high
    ...
    uint8_t gxromPrg = 0;
    uint8_t gxromChr = 0;
    // Mapper 70 (Bandai discrete): 16KB PRG bank at $8000, last bank fixed at $C000; 8KB CHR.
    uint8_t bandai70Prg = 0;
    uint8_t bandai70Chr = 0;
    // Mapper 71 (Camerica/Codemasters): 16KB PRG bank at $8000, last bank fixed at $C000.
    ...
    bool loadFromFile(const std::string& path) {
        ...
        if (!f || header[0] != 'N' || header[1] != 'E' || header[2] != 'S' || header[3] != 0x1A) {
        ...
        const bool hasHeaderTailData = header[12] || header[13] || header[14] || header[15];
        ...
        if (flags6 & 0x08) mirrorMode = 1; // crude four-screen fallback.
        if (!(mapper == 0 || mapper == 1 || mapper == 2 || mapper == 3 ||
            ...
            // This allows many more commercial/homebrew ROMs to at least load and run basic code.
            // Supported with bankswitching: 0 (NROM), 1 (MMC1), 2 (UxROM), 3 (CNROM),
            // 4 (MMC3), 7 (AxROM), 11 (Color Dreams), 66 (GxROM), 70 (Bandai), 71 (Camerica).
            mapper = 0;
        }
        ...
        f.read(reinterpret_cast<char*>(prg.data()), static_cast<std::streamsize>(prg.size()));
        if (!f) {
        ...
        if (header[5] > 0) {
            f.read(reinterpret_cast<char*>(chr.data()), static_cast<std::streamsize>(chr.size()));
            if (!f) {
    ...
    void resetMapperState() {
        ...
        if (mapper == 7) mirrorMode = 2;
        if (hasPrgRam) std::fill(prgRam.begin(), prgRam.end(), 0);
    }
    ...
    uint8_t cpuRead(uint16_t addr) const {
        if (addr >= 0x6000 && addr < 0x8000) {
            if (hasPrgRam && !prgRam.empty()) return prgRam[addr - 0x6000];
            return 0;
        }
        if (addr < 0x8000 || prg.empty()) return 0;
        switch (mapper) {
            case 1: return cpuReadMmc1(addr);
            case 2: return cpuReadMapper2(addr);
            case 4: return cpuReadMmc3(addr);
            case 7: return cpuRead32kBank(addr, axromPrgBank);
            case 11: return cpuRead32kBank(addr, colorDreamsPrg);
            case 66: return cpuRead32kBank(addr, gxromPrg);
            case 70: return cpuRead16kFixedLast(addr, bandai70Prg);
            case 71: return cpuRead16kFixedLast(addr, camerica71Prg);
            case 3:
            case 0:
            default:
                if (prg.size() == 16384) return prg[(addr - 0x8000) & 0x3FFF];
    ...
    void cpuWrite(uint16_t addr, uint8_t value) {
        if (addr >= 0x6000 && addr < 0x8000) {
            if (hasPrgRam && !prgRam.empty()) prgRam[addr - 0x6000] = value;
            return;
        }
        if (addr < 0x8000) return;
        switch (mapper) {
            case 1: cpuWriteMmc1(addr, value); break;
            case 2:
            ...
            case 3:
                ...
                break;
            case 4: cpuWriteMmc3(addr, value); break;
            case 7:
    ...
    uint8_t ppuRead(uint16_t addr) const {
        ...
        switch (mapper) {
            case 1: return chr[mapMmc1Chr(a) % chr.size()];
            case 4: return chr[mapMmc3Chr(a) % chr.size()];
            case 3:
    ...
    void ppuWrite(uint16_t addr, uint8_t value) {
        if (!hasChrRam || chr.empty()) return;
        const uint16_t a = addr & 0x1FFF;
        switch (mapper) {
            case 1: chr[mapMmc1Chr(a) % chr.size()] = value; break;
            case 4: chr[mapMmc3Chr(a) % chr.size()] = value; break;
            default: chr[a % chr.size()] = value; break;
        }
    }

private:
    // 32KB-bank PRG read used by mappers 7 (AxROM), 11 (Color Dreams), 66 (GxROM).
    ...
    uint8_t cpuReadMmc1(uint16_t addr) const {
        ...
        if (prgMode <= 1) {
            ...
            bank = (addr < 0xC000) ? (mmc1PrgBank % std::max(1, prgBankCount16k)) : (prgBankCount16k - 1);
        }
    ...
    void cpuWriteMmc1(uint16_t addr, uint8_t value) {
        ...
        if (addr < 0xA000) {
            ...
            switch (mmc1Control & 0x03) {
                case 0:
                case 1: mirrorMode = 2; break;
                case 2: mirrorMode = 1; break;
                case 3: mirrorMode = 0; break;
            }
        } else if (addr < 0xC000) {
    ...
    uint8_t cpuReadMmc3(uint16_t addr) const {
        ...
        int bank = 0;
        if (addr < 0xA000) bank = mmc3PrgMode ? secondLast : (mmc3Regs[6] % bank8kCount);
        else if (addr < 0xC000) bank = mmc3Regs[7] % bank8kCount;
        else if (addr < 0xE000) bank = mmc3PrgMode ? (mmc3Regs[6] % bank8kCount) : secondLast;
        else bank = last;
    ...
    size_t mapMmc3Chr(uint16_t addr) const {
        ...
        const uint16_t a = addr & 0x1FFF;
        if (!mmc3ChrMode) {
            if (a < 0x0800) return bankOff(mmc3Regs[0] & 0xFE, a);
            if (a < 0x1000) return bankOff(mmc3Regs[1] & 0xFE, static_cast<uint16_t>(a - 0x0800));
            if (a < 0x1400) return bankOff(mmc3Regs[2], static_cast<uint16_t>(a - 0x1000));
            if (a < 0x1800) return bankOff(mmc3Regs[3], static_cast<uint16_t>(a - 0x1400));
            if (a < 0x1C00) return bankOff(mmc3Regs[4], static_cast<uint16_t>(a - 0x1800));
            return bankOff(mmc3Regs[5], static_cast<uint16_t>(a - 0x1C00));
        }
        if (a < 0x0400) return bankOff(mmc3Regs[2], a);
        if (a < 0x0800) return bankOff(mmc3Regs[3], static_cast<uint16_t>(a - 0x0400));
        if (a < 0x0C00) return bankOff(mmc3Regs[4], static_cast<uint16_t>(a - 0x0800));
        if (a < 0x1000) return bankOff(mmc3Regs[5], static_cast<uint16_t>(a - 0x0C00));
        if (a < 0x1800) return bankOff(mmc3Regs[0] & 0xFE, static_cast<uint16_t>(a - 0x1000));
...
public:
    ...
    void setController(uint8_t state) {
        ...
        if (controllerStrobe) controllerShift = controllerState;
    }
...
public:
    ...
    int step() {
        ...
        switch (op) {
            case 0xA9: A = fetch8(); setZN(A); return 2;           // LDA #imm
            ...
            case 0xBD: { bool p = false; A = read(absX(p)); setZN(A); return 4 + pagePenalty(p); }
            case 0xB9: { bool p = false; A = read(absY(p)); setZN(A); return 4 + pagePenalty(p); }
            case 0xA1: A = read(indX()); setZN(A); return 6;       // LDA (zp,X)
            case 0xB1: { bool p = false; A = read(indY(p)); setZN(A); return 5 + pagePenalty(p); }

            case 0x85: write(zp(), A); return 3;                   // STA zp
            case 0x95: write(zpX(), A); return 4;                  // STA zp,X
            case 0x8D: write(abs(), A); return 4;                  // STA abs
            case 0x9D: write(absX(), A); return 5;                 // STA abs,X
            case 0x99: write(absY(), A); return 5;                 // STA abs,Y
            case 0x81: write(indX(), A); return 6;                 // STA (zp,X)
            case 0x91: write(indY(), A); return 6;                 // STA (zp),Y

            case 0xA2: X = fetch8(); setZN(X); return 2;           // LDX #imm
            ...
            case 0xBC: { bool p = false; Y = read(absX(p)); setZN(Y); return 4 + pagePenalty(p); }
            case 0x84: write(zp(), Y); return 3;                   // STY zp
            case 0x94: write(zpX(), Y); return 4;                  // STY zp,X
            case 0x8C: write(abs(), Y); return 4;                  // STY abs

            case 0xE8: ++X; setZN(X); return 2;                    // INX
            case 0xC8: ++Y; setZN(Y); return 2;                    // INY
            case 0xCA: --X; setZN(X); return 2;                    // DEX
            case 0x88: --Y; setZN(Y); return 2;                    // DEY
            case 0x9A: S = X; return 2;                             // TXS
            case 0xBA: X = S; setZN(X); return 2;                   // TSX
            case 0x4C: PC = abs(); return 3;                       // JMP abs
            case 0x6C: PC = ind(); return 5;                       // JMP (ind)
            case 0x20: jsr(abs()); return 6;                       // JSR
            case 0x60: rts(); return 6;                            // RTS
            case 0x40: rti(); return 6;                            // RTI
            case 0x69: adc(fetch8()); return 2;                    // ADC #imm
            case 0x65: adc(read(zp())); return 3;                  // ADC zp
            case 0x75: adc(read(zpX())); return 4;                 // ADC zp,X
            case 0x6D: adc(read(abs())); return 4;                 // ADC abs
            case 0x7D: { bool p = false; adc(read(absX(p))); return 4 + pagePenalty(p); }
            case 0x79: { bool p = false; adc(read(absY(p))); return 4 + pagePenalty(p); }
            case 0x61: adc(read(indX())); return 6;                // ADC (zp,X)
            case 0x71: { bool p = false; adc(read(indY(p))); return 5 + pagePenalty(p); }
            case 0xE9: sbc(fetch8()); return 2;                    // SBC #imm
            case 0xEB: sbc(fetch8()); return 2;                    // SBC #imm unofficial alias
            case 0xE5: sbc(read(zp())); return 3;                  // SBC zp
            case 0xF5: sbc(read(zpX())); return 4;                 // SBC zp,X
            case 0xED: sbc(read(abs())); return 4;                 // SBC abs
            case 0xFD: { bool p = false; sbc(read(absX(p))); return 4 + pagePenalty(p); }
            case 0xF9: { bool p = false; sbc(read(absY(p))); return 4 + pagePenalty(p); }
            case 0xE1: sbc(read(indX())); return 6;                // SBC (zp,X)
            case 0xF1: { bool p = false; sbc(read(indY(p))); return 5 + pagePenalty(p); }
            case 0x18: P &= ~kFlagC; return 2;                     // CLC
            case 0x38: P |= kFlagC; return 2;                      // SEC
            case 0x58: P &= ~kFlagI; return 2;                     // CLI
            case 0x78: P |= kFlagI; return 2;                      // SEI
            case 0xD8: P &= ~kFlagD; return 2;                     // CLD
            case 0xF8: P |= kFlagD; return 2;                      // SED
            case 0xB8: P &= ~kFlagV; return 2;                     // CLV
            case 0xEA: return 2;                                   // NOP
            case 0x00: brk(); return 7;                            // BRK
            case 0x48: push(A); return 3;                          // PHA
            case 0x68: A = pull(); setZN(A); return 4;             // PLA
            case 0x08: push(P | kFlagB | kFlagU); return 3;        // PHP
            case 0x28: P = (pull() | kFlagU) & ~kFlagB; return 4;  // PLP
            ...
            case 0xD0: return branch((P & kFlagZ) == 0);           // BNE
            case 0xF0: return branch((P & kFlagZ) != 0);           // BEQ
            case 0x10: return branch((P & kFlagN) == 0);           // BPL
            case 0x30: return branch((P & kFlagN) != 0);           // BMI
            case 0x90: return branch((P & kFlagC) == 0);           // BCC
            case 0xB0: return branch((P & kFlagC) != 0);           // BCS
            case 0x70: return branch((P & kFlagV) != 0);           // BVS
            case 0x50: return branch((P & kFlagV) == 0);           // BVC

            case 0x29: andA(fetch8()); return 2;                   // AND #imm
            case 0x25: andA(read(zp())); return 3;                 // AND zp
            case 0x35: andA(read(zpX())); return 4;                // AND zp,X
            case 0x2D: andA(read(abs())); return 4;                // AND abs
            case 0x3D: { bool p = false; andA(read(absX(p))); return 4 + pagePenalty(p); }
            case 0x39: { bool p = false; andA(read(absY(p))); return 4 + pagePenalty(p); }
            case 0x21: andA(read(indX())); return 6;               // AND (zp,X)
            case 0x31: { bool p = false; andA(read(indY(p))); return 5 + pagePenalty(p); }

            case 0x09: ora(fetch8()); return 2;                    // ORA #imm
            case 0x05: ora(read(zp())); return 3;                  // ORA zp
            case 0x15: ora(read(zpX())); return 4;                 // ORA zp,X
            case 0x0D: ora(read(abs())); return 4;                 // ORA abs
            case 0x1D: { bool p = false; ora(read(absX(p))); return 4 + pagePenalty(p); }
            case 0x19: { bool p = false; ora(read(absY(p))); return 4 + pagePenalty(p); }
            case 0x01: ora(read(indX())); return 6;                // ORA (zp,X)
            case 0x11: { bool p = false; ora(read(indY(p))); return 5 + pagePenalty(p); }

            case 0x49: eor(fetch8()); return 2;                    // EOR #imm
            case 0x45: eor(read(zp())); return 3;                  // EOR zp
            case 0x55: eor(read(zpX())); return 4;                 // EOR zp,X
            case 0x4D: eor(read(abs())); return 4;                 // EOR abs
            case 0x5D: { bool p = false; eor(read(absX(p))); return 4 + pagePenalty(p); }
            case 0x59: { bool p = false; eor(read(absY(p))); return 4 + pagePenalty(p); }
            case 0x41: eor(read(indX())); return 6;                // EOR (zp,X)
            case 0x51: { bool p = false; eor(read(indY(p))); return 5 + pagePenalty(p); }

            case 0xC9: cmp(fetch8()); return 2;                    // CMP #imm
            case 0xC5: cmp(read(zp())); return 3;                  // CMP zp
            case 0xD5: cmp(read(zpX())); return 4;                 // CMP zp,X
            case 0xCD: cmp(read(abs())); return 4;                 // CMP abs
            case 0xDD: { bool p = false; cmp(read(absX(p))); return 4 + pagePenalty(p); }
            case 0xD9: { bool p = false; cmp(read(absY(p))); return 4 + pagePenalty(p); }
            case 0xC1: cmp(read(indX())); return 6;                // CMP (zp,X)
            case 0xD1: { bool p = false; cmp(read(indY(p))); return 5 + pagePenalty(p); }
            case 0xE0: cpx(fetch8()); return 2;                    // CPX #imm
            case 0xE4: cpx(read(zp())); return 3;                  // CPX zp
            case 0xEC: cpx(read(abs())); return 4;                 // CPX abs
            case 0xC0: cpy(fetch8()); return 2;                    // CPY #imm
            case 0xC4: cpy(read(zp())); return 3;                  // CPY zp
            case 0xCC: cpy(read(abs())); return 4;                 // CPY abs
            case 0xE6: inc(zp()); return 5;                        // INC zp
            case 0xF6: inc(zpX()); return 6;                       // INC zp,X
            case 0xEE: inc(abs()); return 6;                       // INC abs
            case 0xFE: inc(absX()); return 7;                      // INC abs,X
            case 0xC6: dec(zp()); return 5;                        // DEC zp
            case 0xD6: dec(zpX()); return 6;                       // DEC zp,X
            case 0xCE: dec(abs()); return 6;                       // DEC abs
            case 0xDE: dec(absX()); return 7;                      // DEC abs,X
            case 0x0A: aslA(); return 2;                           // ASL A
            case 0x06: asl(zp()); return 5;                        // ASL zp
            case 0x16: asl(zpX()); return 6;                       // ASL zp,X
            case 0x0E: asl(abs()); return 6;                       // ASL abs
            case 0x1E: asl(absX()); return 7;                      // ASL abs,X
            case 0x4A: lsrA(); return 2;                           // LSR A
            case 0x46: lsr(zp()); return 5;                        // LSR zp
            case 0x56: lsr(zpX()); return 6;                       // LSR zp,X
            case 0x4E: lsr(abs()); return 6;                       // LSR abs
            case 0x5E: lsr(absX()); return 7;                      // LSR abs,X
            case 0x2A: rolA(); return 2;                           // ROL A
            case 0x26: rol(zp()); return 5;                        // ROL zp
            case 0x36: rol(zpX()); return 6;                       // ROL zp,X
            case 0x2E: rol(abs()); return 6;                       // ROL abs
            case 0x3E: rol(absX()); return 7;                      // ROL abs,X
            case 0x6A: rorA(); return 2;                           // ROR A
            case 0x66: ror(zp()); return 5;                        // ROR zp
            case 0x76: ror(zpX()); return 6;                       // ROR zp,X
            case 0x6E: ror(abs()); return 6;                       // ROR abs
            case 0x7E: ror(absX()); return 7;                      // ROR abs,X
            case 0x24: bit(read(zp())); return 3;                  // BIT zp
            case 0x2C: bit(read(abs())); return 4;                 // BIT abs

            // ---- Unofficial / illegal opcodes (used by commercial ROMs) ----
            // 1-byte implied NOPs.
            case 0x1A: case 0x3A: case 0x5A: case 0x7A:
            case 0xDA: case 0xFA: return 2;
            ...
            case 0x14: case 0x34: case 0x54: case 0x74: case 0xD4: case 0xF4:
                fetch8(); return 4;
            // 3-byte abs NOP.
            case 0x0C: fetch16(); return 4;
            // 3-byte abs,X NOPs.
            case 0x1C: case 0x3C: case 0x5C: case 0x7C: case 0xDC: case 0xFC:
                fetch16(); return 4;

            // LAX (load A and X from same address).
            case 0xA3: lax(indX()); return 6;        // LAX (zp,X)
            case 0xA7: lax(zp()); return 3;          // LAX zp
            case 0xAF: lax(abs()); return 4;         // LAX abs
            case 0xB3: lax(indY()); return 5;        // LAX (zp),Y
            case 0xB7: lax(zpY()); return 4;         // LAX zp,Y
            case 0xBF: lax(absY()); return 4;        // LAX abs,Y

            // SAX (store A & X).
            case 0x83: sax(indX()); return 6;        // SAX (zp,X)
            case 0x87: sax(zp()); return 3;          // SAX zp
            case 0x8F: sax(abs()); return 4;         // SAX abs
            case 0x97: sax(zpY()); return 4;         // SAX zp,Y

            // DCP (DEC + CMP A).
            case 0xC3: dcp(indX()); return 8;
            case 0xC7: dcp(zp()); return 5;
            case 0xCF: dcp(abs()); return 6;
            case 0xD3: dcp(indY()); return 8;
            case 0xD7: dcp(zpX()); return 6;
            case 0xDB: dcp(absY()); return 7;
            case 0xDF: dcp(absX()); return 7;

            // ISB / ISC (INC + SBC).
            case 0xE3: isb(indX()); return 8;
            case 0xE7: isb(zp()); return 5;
            case 0xEF: isb(abs()); return 6;
            case 0xF3: isb(indY()); return 8;
            case 0xF7: isb(zpX()); return 6;
            case 0xFB: isb(absY()); return 7;
            case 0xFF: isb(absX()); return 7;

            // SLO / RLA / SRE / RRA are common in commercial titles.
            case 0x03: slo(indX()); return 8;
            case 0x07: slo(zp()); return 5;
            case 0x0F: slo(abs()); return 6;
            case 0x13: slo(indY()); return 8;
            case 0x17: slo(zpX()); return 6;
            case 0x1B: slo(absY()); return 7;
            case 0x1F: slo(absX()); return 7;
            case 0x23: rla(indX()); return 8;
            case 0x27: rla(zp()); return 5;
            case 0x2F: rla(abs()); return 6;
            case 0x33: rla(indY()); return 8;
            case 0x37: rla(zpX()); return 6;
            case 0x3B: rla(absY()); return 7;
            case 0x3F: rla(absX()); return 7;
            case 0x43: sre(indX()); return 8;
            case 0x47: sre(zp()); return 5;
            case 0x4F: sre(abs()); return 6;
            case 0x53: sre(indY()); return 8;
            case 0x57: sre(zpX()); return 6;
            case 0x5B: sre(absY()); return 7;
            case 0x5F: sre(absX()); return 7;
            case 0x63: rra(indX()); return 8;
            case 0x67: rra(zp()); return 5;
            case 0x6F: rra(abs()); return 6;
            case 0x73: rra(indY()); return 8;
            case 0x77: rra(zpX()); return 6;
            case 0x7B: rra(absY()); return 7;
            case 0x7F: rra(absX()); return 7;

            default: fetch8(); return 2;                           // Unknown: skip operand to avoid desync
        }
    }
...
private:
    ...
    uint16_t fetch16() { const uint8_t lo = fetch8(); return static_cast<uint16_t>(lo | (fetch8() << 8)); }
    ...
    uint16_t indX() { const uint8_t ptr = static_cast<uint8_t>(fetch8() + X); return static_cast<uint16_t>(read(ptr) | (read(static_cast<uint8_t>(ptr + 1)) << 8)); }
    uint16_t indY() { const uint8_t ptr = fetch8(); const uint16_t base = static_cast<uint16_t>(read(ptr) | (read(static_cast<uint8_t>(ptr + 1)) << 8)); return static_cast<uint16_t>(base + Y); }
    uint16_t ind() { const uint16_t ptr = fetch16(); const uint8_t lo = read(ptr); const uint8_t hi = read(static_cast<uint16_t>((ptr & 0xFF00) | ((ptr + 1) & 0x00FF))); return static_cast<uint16_t>(lo | (hi << 8)); }
    ...
    uint16_t indY(bool& crossed) {
        ...
        const uint16_t base = static_cast<uint16_t>(read(ptr) | (read(static_cast<uint8_t>(ptr + 1)) << 8));
        const uint16_t addr = static_cast<uint16_t>(base + Y);
        crossed = (base & 0xFF00) != (addr & 0xFF00);
        return addr;
    }
    ...
    void setZN(uint8_t v) {
        if (v == 0) P |= kFlagZ; else P &= ~kFlagZ;
        if (v & 0x80) P |= kFlagN; else P &= ~kFlagN;
    }
    ...
    void adc(uint8_t v) {
        ...
        if (sum > 0xFF) P |= kFlagC; else P &= ~kFlagC;
        if ((~(A ^ v) & (A ^ result) & 0x80) != 0) P |= kFlagV; else P &= ~kFlagV;
        A = result;
        setZN(A);
    }
    ...
    void aslA() {
        if (A & 0x80) P |= kFlagC; else P &= ~kFlagC;
        A <<= 1;
        setZN(A);
    }
    ...
    void asl(uint16_t a) {
        uint8_t v = read(a);
        if (v & 0x80) P |= kFlagC; else P &= ~kFlagC;
        v <<= 1;
        write(a, v);
        setZN(v);
    }
    ...
    void lsrA() {
        if (A & 0x01) P |= kFlagC; else P &= ~kFlagC;
        A >>= 1;
        setZN(A);
    }
    ...
    void lsr(uint16_t a) {
        uint8_t v = read(a);
        if (v & 0x01) P |= kFlagC; else P &= ~kFlagC;
        v >>= 1;
        write(a, v);
        setZN(v);
    }
    ...
    void bit(uint8_t v) {
        if (v & 0x80) P |= kFlagN; else P &= ~kFlagN;
        if (v & 0x40) P |= kFlagV; else P &= ~kFlagV;
        if ((A & v) == 0) P |= kFlagZ; else P &= ~kFlagZ;
    }
    ...
    void sax(uint16_t a) {
        write(a, static_cast<uint8_t>(A & X));
    }
...
class Ppu2C02 {
public:
    explicit Ppu2C02(Cartridge& c) : cart(c) {
    ...
    void clock() {
        const bool visibleOrPrerender = (scanline >= 0 && scanline < 240) || scanline == 261;
        ...
        if (visibleOrPrerender && rendering) {
            if (scanline == 261 && cycle >= 280 && cycle <= 304) {
                ...
                vramAddr = static_cast<uint16_t>((vramAddr & 0x841F) | (tempAddr & 0x7BE0));
            }
            if (cycle == 257) {
                // Horizontal bits transfer t -> v (visible + pre-render).
                vramAddr = static_cast<uint16_t>((vramAddr & 0xFBE0) | (tempAddr & 0x041F));
            }
    ...
    void cpuWriteRegister(uint16_t addr, uint8_t value) {
        switch (addr & 0x2007) {
            case 0x2000:
                ppuctrl = value;
                tempAddr = static_cast<uint16_t>((tempAddr & 0xF3FF) | ((value & 0x03) << 10));
                break;
            case 0x2001:
                ppumask = value;
                break;
            ...
            case 0x2005:
                if (!addrLatch) {
                    ...
                    tempAddr = static_cast<uint16_t>((tempAddr & 0x8FFF) | ((value & 0x07) << 12));
                    tempAddr = static_cast<uint16_t>((tempAddr & 0xFC1F) | ((value & 0xF8) << 2));
                }
                addrLatch = !addrLatch;
                break;
            case 0x2006:
                if (!addrLatch) {
                    tempAddr = static_cast<uint16_t>((tempAddr & 0x00FF) | ((value & 0x3F) << 8));
                } else {
    ...
    uint8_t cpuReadRegister(uint16_t addr) {
        switch (addr & 0x2007) {
            ...
            case 0x2007: {
                ...
                dataBuffer = v;
                if ((vramAddr & 0x3FFF) >= 0x3F00) out = v;
                vramAddr = static_cast<uint16_t>(vramAddr + ((ppuctrl & 0x04) ? 32 : 1));
                return out;
            }
            default:
    ...
    // OAM DMA: write one byte to OAM[oamAddr] and post-increment oamAddr (wraps at 256).
    // Called 256 times by the bus when CPU writes to $4014.
...
private:
    ...
    void renderFrame() {
        ...
        const uint16_t baseNt = static_cast<uint16_t>(0x2000 + ((tempAddr >> 10) & 0x03) * 0x0400);
        const uint16_t bgPatternBase = (ppuctrl & 0x10) ? 0x1000 : 0x0000;
        const int scrollX = ((tempAddr & 0x001F) << 3) | fineX;
        const int scrollY = (((tempAddr >> 5) & 0x001F) << 3) | ((tempAddr >> 12) & 0x07);
        uint8_t universalBg = ppuRead(0x3F00);
        if (universalBg == 0) universalBg = 0x0F;

        // Fast background path for common games: nametable + attribute + pattern decode.
        for (int y = 0; y < kNesHeight; ++y) {
            for (int x = 0; x < kNesWidth; ++x) {
                ...
                const int tableY = 0;
                const uint16_t ntBase = static_cast<uint16_t>(baseNt ^ (tableX ? 0x0400 : 0) ^ (tableY ? 0x0800 : 0));
                const int tileX = (sx >> 3) & 31;
                const int tileY = (sy >> 3) & 29;
                const int tx = sx & 7;
                const int ty = sy & 7;
                const uint16_t ntAddr = static_cast<uint16_t>(ntBase + tileY * 32 + tileX);
                const uint8_t tileIndex = ppuRead(ntAddr);

                const uint16_t attrAddr = static_cast<uint16_t>(ntBase + 0x03C0 + (tileY / 4) * 8 + (tileX / 4));
                ...
                const uint16_t pattAddr = static_cast<uint16_t>(bgPatternBase + tileIndex * 16 + ty);
                ...
                if (bgOpaque) {
                    const uint16_t palAddr = static_cast<uint16_t>(0x3F00 + (paletteSel * 4 + col));
                ...
                sampleSpritePixel(x, y, bgOpaque, spriteNesColor, spriteVisible, spriteBehindBg, sprite0HitHere);
    ...
    void sampleSpritePixel(
        int x, int y, bool bgOpaque, uint8_t& nesColorOut, bool& visibleOut, bool& behindBgOut, bool& sprite0HitOut
    ) {
        visibleOut = false;
        behindBgOut = false;
        sprite0HitOut = false;
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            if (x < sx || x >= (sx + 8) || y < sy || y >= (sy + spriteHeight)) continue;
            ...
            if (sprite8x16) {
                spritePatternBase = static_cast<uint16_t>((tile & 0x01) ? 0x1000 : 0x0000);
            ...
            const uint16_t addr = static_cast<uint16_t>(spritePatternBase + spriteTile * 16 + row);
...
void Bus::attachPpu(Ppu2C02* p) {
    ppu = p;
}

bool Bus::pollNmi() {
    return ppu ? ppu->pullNmi() : false;
}
...
uint8_t Bus::read(uint16_t addr) {
    ...
    if (addr < 0x4000) {
        return ppu ? ppu->cpuReadRegister(static_cast<uint16_t>(0x2000 | (addr & 0x0007))) : 0;
    }
    if (addr == 0x4016) {
...
void Bus::write(uint16_t addr, uint8_t value) {
    ...
    if (addr < 0x4000) {
        if (ppu) ppu->cpuWriteRegister(static_cast<uint16_t>(0x2000 | (addr & 0x0007)), value);
        return;
    }
    if (addr == 0x4014) {
...
class NesCore {
public:
    NesCore() : bus(cart), cpu(bus), ppu(cart) {
        bus.attachPpu(&ppu);
    }

    bool loadRom(const std::string& path) {
        if (!cart.loadFromFile(path)) return false;
        bus.reset();
        cpu.reset();
        ppu.reset();
        romPath = path;
    ...
    void runFrame(bool running) {
        if (!running || !cart.loaded) return;
        int cpuCycles = 0;
        while (cpuCycles < 29780) {
            const int c = cpu.step();
            cpuCycles += c;
            for (int i = 0; i < c * 3; ++i) ppu.clock();
        }
    }
...
void drawText(SDL_Renderer* ren, TTF_Font* font, const std::string& text, int x, int y, SDL_Color color) {
...
void drawButton(SDL_Renderer* ren, TTF_Font* font, const Button& b) {
    ...
    drawText(ren, font, b.label, b.rect.x + 10, b.rect.y + 10, SDL_Color{126, 188, 255, 255});
}

std::string pickRomOnMac() {
#if defined(__APPLE__)
    const char* cmd =
        "osascript -e 'set f to choose file with prompt \"Select a NES ROM\" of type {\"nes\"}' "
        "-e 'POSIX path of f'";
    FILE* p = popen(cmd, "r");
    if (!p) return {};
    char buf[2048];
...
int main(int argc, char** argv) {
    ...
    TTF_Font* font = TTF_OpenFont("/System/Library/Fonts/Supplemental/Arial Unicode.ttf", 14);
    if (!font) {
    ...
    SDL_Texture* frameTex = SDL_CreateTexture(
        ren, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, kNesWidth, kNesHeight
    );
    if (!frameTex) {
    ...
    while (!quit) {
        ...
        SDL_UpdateTexture(frameTex, nullptr, nes.frame().data(), kNesWidth * static_cast<int>(sizeof(uint32_t)));
        ...
        drawText(ren, font, "ac's emu nes 1.0", 16, 14, SDL_Color{115, 178, 255, 255});
        drawText(ren, font, "A:X B:Z Sel:RShift Start:Enter", 16, 384, SDL_Color{84, 142, 216, 255});
        drawText(ren, font, "DPad: Arrow Keys", 420, 270, SDL_Color{80, 140, 220, 255});
        drawText(ren, font, "Cmd/Ctrl+O open ROM", 420, 292, SDL_Color{80, 140, 220, 255});
        drawText(ren, font, running ? "State: Running" : "State: Paused", 420, 314, SDL_Color{108, 170, 248, 255});
        drawText(ren, font, nes.loaded() ? "ROM: loaded" : "ROM: none", 420, 336, SDL_Color{108, 170, 248, 255});
        drawText(ren, font, status, 16, 366, SDL_Color{88, 149, 228, 255});

        for (const auto& b : buttons) drawButton(ren, font, b);
